...
abstract class BaseServices {
  ...
  Future<List<Blog>?> fetchBlogLayout({required Map config}) async {
    try {
      ...
      if (enableBoostEngine) {
        return boostBlogLayout(config: dataConfig);
      }
      var list = <Blog>[];
  ...
  Future<List<Blog>?> boostBlogs({
    ...
    try {
      ...
      if (category != null) {
        elasticService.mustList.addMultiMatch(
          query: category,
          fields: [
            'categories',
            'pure_taxonomies.categories.term_id',
          ],
        );
      }
      if (tag != null) {
        elasticService.mustList.addMultiMatch(
          ...
          fields: [
            'tags',
            'pure_taxonomies.tags.term_id',
          ],
        );
      }
      if (search?.isNotEmpty ?? false) {
        elasticService.mustList.addQuery(
      ...
      if (orderBy?.isNotEmpty ?? false) {
        elasticService.addSort(
            key: ElasticClient.convertBlogOrderBy('$orderBy'),
            value: order ?? 'desc');
      }
  ...
  Future<PagingResponse<Blog>>? getBlogs(dynamic cursor) async {
    try {
      if (kBoostEngineConfig.isOptimizeEnable && blogApi.isRoot) {
        var blogs = await boostBlogs(offset: cursor);
  ...
  Future<List<Blog>?> getBlogsByCategory(int? cateId) async {
    try {
      if (kBoostEngineConfig.isOptimizeEnable) {
        return boostBlogs(category: cateId?.toString());
      }
