...
class MessageLookup extends MessageLookupByLibrary {
  ...
  static String m23(number) => " ${number}のグループで購入する必要があります";

  static String m24(itemCount) => "${itemCount}個のアイテム";

  static String m25(price) => "オプションの合計： ${price}";

  static String m26(amount) => " ${amount}を支払う";

  static String m27(total) => "数量： ${total}";

  static String m28(percent) => "セール${percent}％";

  static String m29(second) => "${second}秒前";

  static String m30(totalCartQuantity) =>
      "Shopping cart, ${totalCartQuantity} items";

  static String m31(numberOfUnitsSold) => "販売済み：${numberOfUnitsSold}個";

  static String m32(fieldName) => " ${fieldName} フィールドは必須です";

  static String m33(total) => "${total} 製品";

  static String m34(maxPointDiscount, maxPriceDiscount) =>
      "この注文で最大 ${maxPointDiscount} ポイントを使用すると、 ${maxPriceDiscount} 割引を受けられます。";

  static String m35(date) => " ${date}まで有効";

  static String m36(message) => "警告：${message}";

  static String m37(defaultCurrency) =>
      "現在選択されている通貨はウォレット機能では利用できません。 ${defaultCurrency}に変更してください";

  static String m38(length) => "${length} 個の商品が見つかりました";

  static String m39(week) => "週 ${week}";

  static String m40(name) => "ようこそ ${name}";

  static String m41(year) => "${year} 年前";

  static String m42(total) => "注文番号${total}に割り当てました";
